...
% sobrescreve). Este texto forma a nota de natureza da FOLHA DE ROSTO, então
% mantenha a instituição nele — o Apêndice B do guia exige ("...pela Universidade
% do Vale do Itajaí, Escola de ..."). Na CAPA ele não aparece: o Apêndice A pede
% só o nome da instituição.
...
% Cabeçalho da instituição no TOPO DA CAPA — OPCIONAL: por padrão sai
% "UNIVERSIDADE DO VALE DO ITAJAÍ" (mais o bloco do programa, se PPGCA), que é o
% modelo do Apêndice A. Use apenas se quiser um cabeçalho diferente desse — por
% exemplo incluindo o Centro e o Curso. Substitui o bloco todo. Use \\ para
% quebrar linhas e DIGITE EM MAIÚSCULAS (não há conversão automática de caixa).
...
% (elaborada por bibliotecário(a)). Habilitada aqui para demonstração; na
% qualificação, comente esta linha.
\incluifichacatalografica{tex/pre-textual/ficha-catalografica.tex}

% Folha de aprovação — obrigatória na versão FINAL (após a defesa). Habilitada
% aqui para demonstração; na qualificação (sem defesa), comente esta linha.
...
% Listas (descomente as que for usar; aqui todas as aplicáveis estão ativas para
% demonstração — o template tem figura, tabela, quadro, algoritmo e código)
...
% na "Lista de abreviaturas e siglas", agrupada por A ou S. Exemplos abaixo —
% substitua pelos seus.
%
% A CHAVE da entrada é a própria sigla, então no texto escreva \gls{ABNT}: sai
% "ABNT" com link para a entrada na lista, e na PRIMEIRA vez expande sozinho
% para "Associação Brasileira de Normas Técnicas (ABNT)", como pede a NBR
% 14724:2024, 5.6. Para cadastrar já na primeira menção, sem passar por aqui,
% use \sigla{}{} dentro do capítulo (ver tex/fundamentacao-teorica.tex).
% =====================================================================
...
\input{tex/pre-textual/declaracao-ia}
...
% Referências (obrigatórias) — NBR 6023:2025.
\printbibliography[title={REFERÊNCIAS}]

% Glossário — OPCIONAL e pós-textual, DEPOIS das Referências (NBR 14724:2024).
% Não confundir com a lista de abreviaturas e siglas, que é pré-textual e vem
% dos \nomenclature{} acima. Defina os termos no preâmbulo com
% \newword{termo}{definição} — eles recebem as chaves Def.1, Def.2... na ordem —
% e cite-os no texto com \gls{Def.1} (ou coloque \glsaddall após
% \begin{document} para listar todos). Só entradas usadas são impressas: sem
% nenhuma, esta linha não gera página alguma.
\printglossary

% ----------------------------------------------------------
% APÊNDICES (produção do próprio autor) — OPCIONAL
% Identificados por "APÊNDICE A", "APÊNDICE B"... (letras maiúsculas), título
% centralizado. Cada \chapter vira um apêndice.
% ----------------------------------------------------------
\begin{apendicesenv}
    \chapter{Instrumento de coleta de dados}
    \label{ape:questionario}
    Material elaborado pelo próprio autor (ex.: roteiro de entrevista,
    questionário, código-fonte completo). Lorem ipsum dolor sit amet,
    consectetur adipiscing elit, sed do eiusmod tempor incididunt.
\end{apendicesenv}

% ----------------------------------------------------------
% ANEXOS (material de terceiros) — OPCIONAL
% Identificados por "ANEXO A", "ANEXO B"... Material NÃO elaborado pelo autor
% (ex.: lei, parecer, tabela de terceiros).
% ----------------------------------------------------------
\begin{anexosenv}
    \chapter{Documento de terceiros}
    \label{anx:documento}
    Material não elaborado pelo autor (ex.: legislação, parecer do comitê de
    ética, mapa ou tabela de outra fonte). Ut enim ad minim veniam, quis nostrud
