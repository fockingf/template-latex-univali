...
% longa com o ambiente citacao, grifo nosso, supressão [...] e apud), sigla na
% primeira menção, nota de rodapé, figura, equação, alíneas (ABNT) e listas.
% =====================================================================
...
% SIGLA na PRIMEIRA menção (NBR 14724:2024, 5.6): "A sigla, quando mencionada
% pela primeira vez no texto, deve ser indicada entre parênteses, precedida pelo
% nome completo" — ex.: Associação Brasileira de Normas Técnicas (ABNT). O
% \sigla{}{} faz as duas coisas de uma vez: escreve "nome completo (SIGLA)" aqui
% e cadastra a entrada na lista de abreviaturas e siglas (pré-textual), sem
% precisar repetir o \nomenclature no preâmbulo.
% Nas menções SEGUINTES, use \gls{IBGE}: sai apenas "IBGE", e tanto ele quanto a
% primeira menção ficam com link para a entrada na lista. Se você digitar a sigla
% como texto puro, ela aparece igual no PDF, mas sem link.
% Variações: \sigla*{}{} apenas cadastra (não escreve nada no texto) e
% \sigla[A]{}{} manda a entrada para o grupo "Abreviaturas" em vez de "Siglas".
Os dados populacionais utilizados nesta seção provêm do
\sigla{IBGE}{Instituto Brasileiro de Geografia e Estatística}. As séries
históricas do \gls{IBGE} cobrem o período analisado.
